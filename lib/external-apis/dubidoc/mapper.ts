import { actPdfFilename } from "@/lib/acts/pdf-filename";
import type { Act } from "@/lib/db/schema/acts";

import type { CreateDocumentRequest } from "./types";

interface ActClientSnapshot {
  name: string;
  shortName: string | null;
  email: string;
  legalId: string;
}

interface ActContractSnapshot {
  number: string;
}

export function actToCreateDocumentPayload(act: Act, pdfBase64: string): CreateDocumentRequest {
  const snapshot = act.clientSnapshot as ActClientSnapshot;
  const contract = act.contractSnapshot as ActContractSnapshot;
  // DubiDoc expects `amount` in kopiykas (integer minor units) — sending
  // hryvnias makes the displayed sum 100× too small. Source the real paid total
  // from `act.amount`, not unit_price × quantity (they differ for annual acts).
  const amount = Math.round(Number(act.amount) * 100);

  // Title identifies the counterparty in the DubiDoc list: short name (or full
  // name when none) + bare contract number + the act header. Casing is preserved.
  const displayName = snapshot.shortName?.trim() || snapshot.name;

  return {
    file: pdfBase64,
    filename: actPdfFilename(act),
    title: `${displayName} ${contract.number} Акт ${act.number} від ${act.actDate}`,
    date: act.actDate,
    number: act.number,
    amount,
    signatureType: "external",
    workflowType: "sequential",
    participants: [
      {
        action: "sign",
        email: snapshot.email,
        edrpou: snapshot.legalId,
        priority: 1,
        isSignatureRequired: true,
      },
    ],
  };
}
