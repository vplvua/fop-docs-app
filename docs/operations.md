...
2. **Заповнити реквізити** в адмінці: **Налаштування → Реквізити** (без них акти
   ...
   ⚠️ Поле `bankAccount` у реквізитах — це IBAN для **друку в акті**. Воно НЕ
   замінює env-змінну `FOP_BANK_ACCOUNT`, з якої PrivatBank Автоклієнт API тягне
   виписку платежів (`acc`-параметр, `lib/external-apis/privatbank/poll.ts`).
   Це один фізичний рахунок у двох місцях для двох споживачів — `FOP_BANK_ACCOUNT`
   лишається обовʼязковою env-змінною (NFR-SEC-02), видалення ламає polling.
...
## Зміна назв послуг на актах

Назви послуг (рядок опису на акті) редагуються в адмінці:
**Налаштування → Тарифи** (послуга «доступ») і **Налаштування → Ціни СМС**
(послуга «інтернет-розсилка»). Зберігаються в `settings` під ключем `service_names`;
без значення діють дефолтні формулювання — поведінка незмінна, поки адмін не відредагує.

- **Нові акти** підхоплюють змінену назву одразу.
- **Вже видані акти** зберігають стару назву, доки не запустити
  **«Перегенерувати всі акти»** (Дашборд → «Ручні дії») — перегенерація
  перераховує `service_description` з поточних назв. Той самий runbook і ризики,
  що й вище.

---

## Зміна `add-annual-tariff` — річний платіж зі знижкою (runbook)

Schema-зміна (`acts.amount`, `acts.billing_period`, міграція
`0012_add_act_amount_billing_period`) + розпізнавання річного платежу в
класифікації. Порядок:

1. **Dev:** `npm run db:migrate` (застосовує `0012` на dev-гілці). Міграція додає
   `amount` (бекфіл існуючих рядків = `unit_price × quantity`, далі NOT NULL) і
   `billing_period` (default `monthly`).
2. **Деплой коду** (див. вище).
3. **Production-міграція окремо** — `0012` НЕ застосовується локальним
   `npm run db:migrate`. Виконати рецепт «Міграція на production» з
   prod-`POSTGRES_URL` (additive колонки + бекфіл, безпечно).
4. **Налаштувати знижку** (за потреби): **Налаштування → Тарифи** → «Річний платіж
   (знижка)» — поле «Оплачених місяців за рік» (ключ `settings.annual_paid_months`,
   дефолт **10** = «плати за 10 місяців, отримай 12»). Річна ціна тарифу = місячна
   ціна × це число. **Не діє** для клієнтів з індивідуальною ціною доступу.

Платіж, що точно дорівнює річній ціні (напр. 2000 при тарифі 200), стає актом на
**12 шт.** із сумою = фактичному платежу; `amount` на акті — джерело суми в PDF,
UI та в payload Дубідок (де передається в **копійках**, `round(amount × 100)`).
