...
Скорочення доменів: `AUTH` (auth), `PAY` (платежі), `CLASS` (класифікація), `CLI` (клієнти), `CTR` (договори), `ACT` (акти), `MACT` (ручні акти), `TAR` (тарифи), `EDO` (ЕДО), `SYNC` (sync), `QUEUE` (черга), `UI` (інтерфейс), `SET` (налаштування), `PERF`, `SEC`, `LOG`, `AVAIL`, `STACK`, `HOST`, `DB`, `INTEG`, `LEGAL`, `DATA`, `USER`, `SCOPE`.
...
### 3.15 Ручні акти (`FR-MACT-*`, Phase 1)

Ручне створення/редагування/видалення акту для грошей, що не пройшли автоматичну класифікацію ПриватБанку (інший банк або платіж до запуску). Деталі — capability `openspec/specs/manual-acts/spec.md`.

- **FR-MACT-01:** Сторінка «Створити акт вручну» збирає: клієнт (лише ті, що мають договір), період акту (місяць + рік), послуга (`access` / `sms`), кількість, сума. Форма server-валідована (Zod) до створення будь-чого.
- **FR-MACT-02:** При виборі клієнта/послуги ціна за одиницю передзаповнюється з чинного тарифу (`resolveAccessPrice` / `resolveSmsPrice`) як підказка; кількість і суму адмін може перевизначити.
- **FR-MACT-03:** `act_date` ручного акту = останній день обраного місяця періоду, незалежно від `payment_date` backing-платежу.
- **FR-MACT-04:** Створення ручного акту спершу створює backing-`payments` рядок: `source = 'manual_external'`, синтетичний `bank_transaction_id = 'manual:{uuid}'`, сума адміна, `payment_date`, опц. `bank_label`, `payer_*` з клієнта.
- **FR-MACT-05:** Створений акт посилається на цей платіж через `payment_id` (інваріант «акт завжди має платіж»); платіж лишається `status = 'classified'` з `act_id` нового акту. Синтетичний id ніколи не колізить з ПриватБанк `REF+REFN`.
- **FR-MACT-06:** Шлях ручного створення реюзає race-safe нумерацію (`SELECT ... FOR UPDATE`), FOP-snapshot, генерацію/зберігання PDF і відправку в Дубідок — без повторного виведення service/quantity/amount через автокласифікацію.
- **FR-MACT-07:** Сума й кількість беруться з вводу адміна як є (amount-as-entered, D4) і не перераховуються з тарифу.
- **FR-MACT-08:** Адмін може **відредагувати** ручний акт (backing-платіж `source = 'manual_external'`), поки він мутабельний (`status = 'draft'` АБО `edo_provider = 'vchasno_external'`). Редаговані поля — значущі: `service_type`, `quantity`, `unit_price`, `amount`, `bank_label`, `payment_date` (+ опис послуги). **Клієнт і період незмінні** (вони задають номер акту і snapshots). Акт і backing-платіж оновлюються однією транзакцією (сума синхронізована), після чого PDF перегенеровується. Редагування не-ручного акту, надісланого `sent_to_edo` або підписаного dubidoc-акту — заборонене.
- **FR-MACT-09:** Адмін може **видалити** ручний акт, поки він мутабельний (як у FR-MACT-08): акт і його синтетичний backing-платіж видаляються однією транзакцією (hard delete), без осиротілого `classified`-платежу; номер `MM/YYYY[/N]` звільняється для повторного використання. Видалення не-ручного акту, `sent_to_edo` або підписаного dubidoc-акту — заборонене. (Виняток до BC-DATA-03/NFR-SEC-08: ручний акт із синтетичним платежем не є історією реального банківського платежу.)
