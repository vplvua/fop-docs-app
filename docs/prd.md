...
- **FR-CLASS-02:** Класифікатор виконує кроки в порядку: парсинг номера договору → dedup + multiple_contracts check → матчинг клієнта → перевірка `auto_act_disabled` → перевірка `edo_provider` → визначення `service_type` → перевірка повноти даних → резолв ціни → визначення `quantity` → генерація `Act`.
...
- **FR-CLASS-04:** При знайденні >1 різних номерів договору — `status=in_queue`, `reason=multiple_contracts`; адмін обирає один у UI.
- **FR-CLASS-05:** Матчинг клієнта — двофакторний: номер договору + `client.legal_id == payer_legal_id`. При розбіжності — `in_queue(ambiguous_client)`.
- **FR-CLASS-06:** Для платежів з транзитного рахунку (`payer_legal_id ∈ Settings.transit_edrpou_list`) матчинг тільки за номером договору, без перевірки ЄДРПОУ.
- **FR-CLASS-07:** Якщо клієнт не зматчений за договором — fallback пошук за `payer_legal_id`. Якщо й це не дало результату — `in_queue(no_match)`.
...
- **FR-QUEUE-05:** Для `ambiguous_client` показуються `client.legal_id` і `payer.legal_id` поруч + дії "Підтвердити клієнта", "Створити іншого".
