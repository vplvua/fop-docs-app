...
Operator home screen — the `/` route that surfaces per-integration health banners (ПриватБанк, Дубідок, "Моє ОСББ") sourced from `integration_health`, attention counters for payments and acts that need action, and manual integration triggers that invoke existing server actions and refresh the view. Covers FR-UI-01..03.
