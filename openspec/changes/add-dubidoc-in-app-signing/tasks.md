...
- [x] 6.2 Dev-smoke (2026-06-04, screenshots): акт `sent_to_edo` → «Підписати тут» → підпис ЕЦП в iframe → «Готово» → статус оновився на сторінці деталей. (Примітка: цей акт перейшов одразу в `Підписано`, а не `waiting_for_client_sign` — Дубідок повернув `signed`; мапінг відпрацював коректно.)
