...
export function MoeosbbSyncButton() {
  ...
  const handleSync = useCallback(async () => {
    ...
    if (res.ok && res.result) {
      const { fetched, matched, updated, created, contractsUpdated, contractsCreated } = res.result;
      setMessage(
        `Отримано ${String(fetched)}, знайдено ${String(matched)}, оновлено ${String(updated)}, створено ${String(created)}, договорів оновлено ${String(contractsUpdated)}, договорів створено ${String(contractsCreated)}`,
      );
      router.refresh();
    } else {
