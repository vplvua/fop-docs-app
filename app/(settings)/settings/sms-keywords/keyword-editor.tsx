...
function RemoveChip({ value }: { value: string }) {
  const [, formAction] = useActionState(removeKeyword, initialListActionState);
  return (
    <form action={formAction} className="inline">
      <input type="hidden" name="value" value={value} />
      <button
        type="submit"
        className="inline-flex items-center gap-1 rounded-full border border-border bg-muted/50 px-3 py-1 text-sm text-foreground transition-colors hover:bg-destructive/10 hover:text-destructive"
      >
...
export function KeywordEditor({ keywords }: { keywords: string[] }) {
  const [state, formAction] = useActionState(addKeyword, initialListActionState);

  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        {keywords.map((k) => (
          <RemoveChip key={k} value={k} />
        ))}
        {keywords.length === 0 ? (
          <p className="text-sm text-muted-foreground">Немає ключових слів</p>
        ) : null}
      </div>
